function frontImpactSF = FrontImpact_3DFEA(thicknessPrimary, thicknessSecondary, maxForce)
    ...
    Nodal_coordinates(2,1) = -1354/25.4;        % x Coordinate, Node 1
    Nodal_coordinates(2,2) = 213.32/25.4;       % y Coordinate, Node 1
    Nodal_coordinates(2,3) = 241.32/25.4;

    % The same comments apply for the rest of the nodes
    ...
    Connect_table(2,1) = 2;  % Node i for element 2 
    Connect_table(2,2) = 47; % Node j for element 2 
    Connect_table(2,3) = 1;  % Material flag
 
    % The same comments apply for the rest of the elements
    Connect_table(3,1) = 3;
    ...
    % Element inner radii
    Innerradiielementtype1 = Outerradiielementtype1 - thicknessPrimary/25.4;   % inches
    Innerradiielementtype2 = Outerradiielementtype2 - thicknessSecondary/25.4; % inches

    % Element moment of Inertia
    inertiaelementtype1 = (pi/4)*((Outerradiielementtype1^4) - (Innerradiielementtype1^4));  % inches^4
    inertiaelementtype2 = (pi/4)*((Outerradiielementtype2^4) - (Innerradiielementtype2^4));  % inches^4

    % Polar moment of Inertia
    polarelementype1 = (pi/2)*((Outerradiielementtype1^4) - (Innerradiielementtype1^4)); % inches^4
    polarelementype2 = (pi/2)*((Outerradiielementtype2^4) - (Innerradiielementtype2^4)); % inches^4
    ...
    % Front impact forces (in x direction) at nodes 1,2,3,10,11,12

    Fa(1,1) = (maxForce/4.44822)/6;
    Fa(7,1) = (maxForce/4.44822)/6;
    Fa(13,1) = (maxForce/4.44822)/6;
    Fa(55,1) = (maxForce/4.44822)/6;
    Fa(61,1) = (maxForce/4.44822)/6;
    Fa(67,1) = (maxForce/4.44822)/6;

    % Boundary conditions, nodes 39,40,41,42 are all fixed in all directions
    BC = zeros(nnode,ndof_per_node);
    BC(39,1:6) = 1;    % 1 means fixed dof
    BC(40,1:6) = 1;
    BC(41,1:6) = 1;
    BC(42,1:6) = 1;
    ...
    % Calculating mass of chassis
    for i = 1:nelem

        Volume(i,1) = A(i,1)*L(i,1);
        mass(i,1) = Volume(i,1)*Density(1,1);
    end
    ...
    % Mass Calculation
    total_chassis_mass = sum(mass)*0.453592;
    sum(L);
    Density;


    % Stress calculations
    [tensile, in1] = min(norm_stress_1*0.00689476);
    [compressive, in2] = max(norm_stress_1*0.00689476);
    [shear_y, in3] = max(abs(shear_stress_y*0.00689476));
    [shear_z, in4] = max(abs(shear_stress_z*0.00689476));
    [torsion, in5] = max(abs(Torsion_stress_x*0.00689476));
    [bending_y, in6] = max(abs(bending_stress_y*0.00689476));
    [bending_z, in7] = max(abs(bending_stress_z*0.00689476));
    ...
    [minSF, failureIndex] = min(n_vector);
    
    if A(failureIndex,1) == Outerradiielementtype1 
        failureElementType = 1;
    else
        failureElementType = 2;
    end
