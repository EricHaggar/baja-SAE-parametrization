function rearImpactSF = RearImpact_3DFEA(thicknessPrimary, thicknessSecondary, impactForce)
    ...
    Nodal_coordinates(2,1) = -1354/25.4;     % x Coordinate, Node 2
    Nodal_coordinates(2,2) = 213.32/25.4;    % y Coordinate
    Nodal_coordinates(2,3) = 241.32/25.4;    % z Coordinate 
    ...
    % Element outer radii
    Outerradiielementtype1 = (outerDiameterPrimary/2)/25.4;   % mm
    Outerradiielementtype2 = (outerDiameterSecondary/2)/25.4; % mm

    % Element inner radii
    Innerradiielementtype1 = Outerradiielementtype1 - thicknessPrimary/25.4;    % mm^4
    Innerradiielementtype2 = Outerradiielementtype2 - thicknessSecondary/25.4;  % mm^4

    % Element moment of Inertia
    inertiaelementtype1 = (pi/4)*((Outerradiielementtype1^4) - (Innerradiielementtype1^4)); % mm^4
    inertiaelementtype2 = (pi/4)*((Outerradiielementtype2^4) - (Innerradiielementtype2^4)); % mm^4

    % Polar moment of Inertia
    polarelementype1 = (pi/2)*((Outerradiielementtype1^4) - (Innerradiielementtype1^4)); % mm^4
    polarelementype2 = (pi/2)*((Outerradiielementtype2^4) - (Innerradiielementtype2^4)); % mm^4
    ...
    % Front impact forces (in z direction) at node 39, 40, 41 and 42
    Fa(229,1) = -(impactForce/4.44822)/4; 
    Fa(235,1) = -(impactForce/4.44822)/4;
    Fa(241,1) = -(impactForce/4.44822)/4;
    Fa(247,1) = -(impactForce/4.44822)/4;


    % Boundary conditions, nodes 1,2,3,10,11,12 are all fixed in all directions
    BC = zeros(nnode,ndof_per_node);
    BC(1,1:6) = 1; % 1 means fixed dof
    BC(2,1:6) = 1;
    BC(3,1:6) = 1;
    BC(10,1:6) = 1;
    BC(11,1:6) = 1;
    ...
    % Stress calculations. The maxiumum values are found to calculate safety factors
    ...
    % Put the safety factors in an array
    n_vector = [n_tensile;n_compressive;n_shear_y;n_shear_z;n_torsion;n_bending_y;n_bending_z];
    
    % Minimum safety factor is returned
    rearImpactSF = min(n_vector);
